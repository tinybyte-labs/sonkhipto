...
export const getLatestArticleLinksFromDailyStart: GetLatestArticleLinksFn =
    async (browser) => {
...
export const getArticleMetadataFromDailyStart: GetArticleMetadataFn =
    async (articleUrl, browser) => {
        ...
        return {
            ...
            publishedAt: metadata.pubDate ? new Date(metadata.pubDate.trim().split("Last update on:")[0].trim()) : null,
        };
    };
